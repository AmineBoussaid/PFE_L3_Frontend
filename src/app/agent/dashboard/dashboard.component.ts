...
export class DashboardComponent implements OnInit {
  ...
  calculateStatistics(): void {
    ...
    this.terminees = this.reclamations.filter(r => r.status === 'Terminee').length;
  ...
  getQuartierData() {
    ...
    this.reclamations.forEach(reclamation => {
      if (reclamation.quartier) {
        const quartier = reclamation.quartier;
  ...
  if (ctxMonthly) {
    this.monthlyChart = new Chart(ctxMonthly, {
      ...
      data: {
        ...
        datasets: [{
          label: `Réclamations par Mois (${this.selectedYear})`,
